...
- [x] Implement the environment-backed initial `ADMIN` seed, BCrypt hashing, a 12-character minimum password policy, and secret-safe logging/redaction. Make bootstrap idempotent under restart/concurrent startup; never overwrite an existing account's password, role, or status or promote an email collision.
- [x] Configure form login/logout and route rules: only login, invitation acceptance, password reset, static assets, and necessary error pages are anonymous; every other request requires an `ACTIVE` account.
- [x] Implement successful-login timestamp updates and throttling by normalized identity plus request source without revealing whether an email exists.
- [x] Implement random hashed invitation tokens with the `PENDING`, `ACCEPTED`, `EXPIRED`, and `REVOKED` lifecycle, seven-day validity, single use, and atomic resend behavior that revokes the prior pending invitation before issuing a new token. Serialize acceptance/resend for a normalized email, allow at most one pending invitation across customer/employee roles, and derive the activated role from the stored invitation rather than submitted form data. GET requests only display forms; token consumption and account creation require a validated POST.
- [x] Implement the production SMTP adapter behind the existing notifications `MailDelivery` interface and Polish activation/reset templates using the configured application base URL. Send links only after the token transaction commits, store only token hashes, and handle delivery failure with a usable resend/reset-request path without exposing account existence or persisting/logging raw links or email bodies containing tokens.
- [x] Deliver the administrator employee-account slice: staff directory, `EMPLOYEE` invitation email, activation/password creation without a customer profile, and administrator-only block/unblock actions. Record invitation and account-status audit events with the actor.
- [x] Implement forgotten-password email flow with random hashed one-hour tokens, single-use consumption, password replacement, security-version rotation, and invalidation of all existing sessions. Consume tokens and replace the password atomically, invalidate outstanding reset tokens on password replacement, use generic request responses, and never unblock a blocked account through password recovery.
- [x] Implement a shared block/unblock service that restricts EMPLOYEE actors to CUSTOMER targets and ADMIN actors to CUSTOMER/EMPLOYEE targets. Atomically update status, rotate the security version on blocking, and record the audit event; reject stale sessions on every authenticated full-page/HTMX request, including after unblock, and prevent a concurrent login from retaining access with a stale security version.
- [x] Implement authenticated password change with current-password verification and the same session invalidation guarantees; expose no self-service role, email, company, or billing-data changes.
...
- [ ] Verify with unit/integration/MVC tests: bootstrap restart/collision behavior, normalization and uniqueness races, token expiry/replay/resend and role tampering, password rules and hashing, SMTP failure recovery, throttling, anonymous-route boundaries, role checks, blocked-login rejection, login/block/reset races, blocked-account recovery, stale-session rejection after unblock, and CSRF failures.
...
- [ ] Implement employee/admin resend and customer block/unblock actions through the Phase 2 services; retain the company/billing payload in the replacement invitation and preserve profiles and historical references when accounts are blocked. Record customer invitation and account-status audit events.
...
- [ ] Implement product, SKU, category assignment, variant-attribute, active-state, and base-price/VAT application services with validation and history-safe behavior: records referenced by orders are inactivated rather than destructively removed. Require at least one SKU before activating a product; reject cart additions and orders whenever either the product or SKU is inactive. Attribute values must belong to the assigned definition and never carry prices.
...
- [ ] Build staff price-list pages to name lists, maintain selected SKU prices, assign/unassign at most one list per customer, and create/update/remove individual customer/SKU overrides and list entries so staff can restore fallback pricing.
...
- [ ] Revalidate current product/SKU activity, price, VAT, and availability whenever checkout is displayed; show actionable line errors and prevent empty-cart checkout. Issue an unpredictable customer/cart-bound one-time token for a server-stored review containing the cart revision, SKU identifiers, quantities, effective unit net prices, and VAT rates; never trust client-supplied totals or fingerprints. An updated review requires explicit customer submission again.
...
- [ ] Implement concurrency-safe `ORD-YYYY-NNNNNN` allocation using the submission year in `Europe/Warsaw` and the injectable clock; test year rollover and preserve uniqueness without requiring gapless numbering. Implement the order aggregate so only intake status/update metadata can change after submission; do not expose mutation or cancellation operations to customers.
- [ ] Implement one transaction that locks/claims the active cart and token, verifies the active customer and exact non-empty reviewed cart revision, re-resolves product/SKU activity/effective prices/VAT, secures stock in a deterministic SKU order, verifies quantities, creates the order/items and billing/delivery snapshots, reduces stock and advances SKU versions, completes the cart, consumes the token, and commits a `SUBMITTED` order visible to staff queries. Record stock reductions with the ordering actor, order reference, previous/new quantities, and timestamp; retain separate last-manual-editor metadata for the inventory page.
- [ ] Define and implement a consistent transaction/locking protocol across submission, cart edits, account blocking, profile edits, product/SKU activation, pricing changes, and inventory updates. Final revalidation and snapshot creation must use protected current state through commit, including price-list assignment changes and concurrent insertion/removal of an override; a read followed by an unprotected write is insufficient. Capture the complete current billing profile atomically through customer application services and preserve acyclic module dependencies.
- [ ] Reject stale reviewed cart contents, prices, or VAT before order creation and return the customer to a newly calculated review; reject insufficient concurrent stock with exact current quantities. Roll back before loading the fresh review so stale transaction state is not displayed. No failure path may create a partial order, consume a usable token, complete the cart, or make stock negative; separately invalidate superseded reviews and require renewed customer confirmation without silently retrying changed commercial terms.
...
- [ ] Verify exact calculations/snapshots, number uniqueness/year rollover, customer history isolation, hidden billing data, token replay across the same and different reviews of one cart, empty-cart rejection, rollback behavior, and cart rollover. Use real PostgreSQL concurrent transactions to cover overlapping multi-SKU orders, manual inventory updates versus submission, cart edits, account blocking, profile edits, product/SKU inactivation, and base/list/override/VAT/assignment changes; verify atomic snapshots, no lost stock changes, and renewed review when commercial values change.
...
- [ ] Add migrations for notification outbox/delivery attempts, including a unique order/notification-type/recipient key, recipient, retry state, attempt timestamps, worker claim/lease data, sanitized failure details, and indexes for due work and permanent failures. Track delivery independently per recipient so one failed staff address does not resend successful notifications.
...
- [ ] Implement customer and staff Polish email templates rendered from immutable order snapshots. Include public order number, order/price summary, authenticated application link, and optional purchase-order number; keep billing/profile data out of customer mail, and include full delivery/contact data (but link to the application for billing data) in mail to every account that was an active `EMPLOYEE` or `ADMIN` at acceptance. Address recipients individually to avoid disclosing other accounts' email addresses.
- [ ] Trigger asynchronous dispatch immediately after commit without holding up the confirmation response on SMTP. Implement bounded retry/backoff for temporary failures, permanent-failure classification (including exhausted retries), and safe logging. Atomically claim due recipient jobs across dispatchers, recover abandoned claims after crashes, and let the scheduled worker recover work when the after-commit signal is lost. Deduplicate notification jobs and completed deliveries; document that a crash after SMTP acceptance but before recording success can still cause duplicate email, since SMTP does not guarantee exactly-once delivery.
- [ ] Build the shared paginated staff inbox with status filter, complete order detail including immutable billing and delivery snapshots, purchase-order number, totals, and per-recipient notification status/permanent-failure visibility for employees/admins.
...
- [ ] Verify recipient selection (including blocked staff and zero active staff), after-commit timing, no notification work after rollback, accepted-order survival during SMTP failure, independent recipient retries, concurrent workers, restart/abandoned-claim recovery, lost dispatch signals, exhausted-retry visibility, and email data boundaries. Verify inbox authorization/filtering/pagination, state-machine conflicts, audit entries, and cancellation stock behavior.
